...
SELECT      LName, FName
FROM        BOOKED AS B, PASSENGER AS P, TRAIN_STATUS, TRAIN
WHERE       TrainDate='Saturday' AND P.SSN=B.SSN AND B.Train_Number=TrainNumber AND TrainName=Train_Name AND Ticket_Status='Booked';
...
WHERE       P.SSN=B.SSN AND B.Train_Number=TrainNumber AND P.BDate IN (
        SELECT BDate
        FROM PASSENGER
        WHERE DATE('now') >= DATE(BDate, '+50 years') AND DATE('now') < DATE(BDate, '+61 years')
    );
